...
public class SteamManager : MonoBehaviour
{
	...
	public async void JoinLobby(Lobby lobby)
	{
		...
		else
		{
			...
			this.leaveLobby();
			TaskAwaiter<RoomEnter> taskAwaiter = lobby.Join().GetAwaiter();
			if (!taskAwaiter.IsCompleted)
			{
				await taskAwaiter;
				TaskAwaiter<RoomEnter> taskAwaiter2;
				taskAwaiter = taskAwaiter2;
				taskAwaiter2 = default(TaskAwaiter<RoomEnter>);
			}
			if (taskAwaiter.GetResult() != RoomEnter.Success)
			{
	...
	private async void OnGameLobbyJoinRequestedCallback(Lobby joinedLobby, SteamId id)
	{
		...
		else
		{
			LocalClient.serverOwner = false;
			this.leaveLobby();
			TaskAwaiter<RoomEnter> taskAwaiter = joinedLobby.Join().GetAwaiter();
			if (!taskAwaiter.IsCompleted)
			{
				await taskAwaiter;
				TaskAwaiter<RoomEnter> taskAwaiter2;
